...
module.exports = class Creator extends EventEmitter {
  ...
  async create(cliOptions = {}, preset = null) {
    ...
    await clearConsole();
    log(chalk.blue.bold(`lzh-vue-cli v${require("../package.json").version}`));
    logWithSpinner(`✨`, `正在创建项目 ${chalk.yellow(name)}.`);
    this.emit("creation", { event: "creating" });

    stopSpinner();
    // 设置文件名，版本号等
    const { pkgName, pkgAuthor, pkgVers, pkgDes } = await inquirer.prompt([
      {
        name: "pkgName",
        message: `请输入项目名称`,
        default: name
      ...
      },
      {
        name: "pkgDes",
        message: `请输入项目简介`,
        default: "project created by lzh-vue-cli"
      }
    ]);
